...
let page = {
  ...
  prepareQuestionActionBtn: function(question) {
    ...
    let questioin_order = question.order || 1;
    ...
    option_install_btns.each(function(index) {
      $(this)
        ...
        .bind("click", function() {
          ...
          let option_html = "";
          // 判断当前的类型
          let question__type = $(this)
            .parents(".question__item")
            .data("qtype");
          // 单选题
  ...
  releaseQuestionnaire: function() {
    ...
    _this.saveQuestionnaire();
    // 调用 store 里的发布问卷函数
    let res = _store.releaseQuestionnaire(_this.data.questionnaire_id);
    // 反馈
    if (res === true) {
      alert("问卷发布成功！");
    } else {
